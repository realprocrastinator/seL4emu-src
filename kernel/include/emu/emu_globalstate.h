...
extern char seL4emu_g_tcbs[CONFIG_MAX_SEL4_CLIENTS][BIT(seL4_TCBBits)] ALIGN(BIT(TCB_SIZE_BITS));
 ...
 * TODO(Jiawei): avoid liner search for performance, but shoudld be fine as we don't have many clients at the moment.
 ...
 * TODO(Jiawei): avoid liner search for performance, but shoudld be fine as we don't have many clients at the moment.
 * return -1 if all full, return slot idx on success.
 */ 
 ...
 * TODO(Jiawei): avoid liner search for performance, but shoudld be fine as we don't have many clients at the moment.
 * return -1 if pid not found, return slot idx on success.
 */ 
 ...
 * TODO(Jiawei): avoid liner search for performance, but shoudld be fine as we don't have many clients at the moment.
 * return 0 if not found, return address found on success.
 */ 
