...
BOOT_CODE bool_t reserve_region(p_region_t reg) {
  ...
  for (i = 0; i < ndks_boot.resv_count; i++) {
    ...
    if (ndks_boot.reserved[i].start > reg.end) {
      ...
      if (ndks_boot.resv_count + 1 >= MAX_NUM_RESV_REG) {
        printf("Can't mark region 0x%lx-0x%lx as reserved, try increasing MAX_NUM_RESV_REG "
               "(currently %d)\n",
  ...
  if (i + 1 == MAX_NUM_RESV_REG) {
    printf("Can't mark region 0x%lx-0x%lx as reserved, try increasing MAX_NUM_RESV_REG (currently "
           "%d)\n",
           reg.start, reg.end, (int)MAX_NUM_RESV_REG);
    return false;
  }
...
BOOT_CODE void create_rootserver_objects(pptr_t start, v_region_t v_reg,
  ...
  // TODO(Jiawei:) we don't handle bi frame at the moment
...
BOOT_CODE cap_t create_root_cnode(void) {
  ...
  cap_t cap = cap_cnode_cap_new(CONFIG_ROOT_CNODE_SIZE_BITS,            /* radix      */
...
#error "Not supported yet!"
    ...
    //     NODE_STATE_ON_CORE(ksIdleThread, i), SC_PTR(&ksIdleThreadSC[SMP_TERNARY(i, 0)]),
...
BOOT_CODE tcb_t *create_initial_thread(cap_t root_cnode_cap, cap_t it_pd_cap, vptr_t ui_v_entry,
  ...
  deriveCap_ret_t dc_ret =
      deriveCap(SLOT_PTR(pptr_of_cap(root_cnode_cap), seL4_CapInitThreadIPCBuffer), ipcbuf_cap);
  ...
  cteInsert(root_cnode_cap, SLOT_PTR(pptr_of_cap(root_cnode_cap), seL4_CapInitThreadCNode),
            SLOT_PTR(rootserver.tcb, tcbCTable));
  cteInsert(it_pd_cap, SLOT_PTR(pptr_of_cap(root_cnode_cap), seL4_CapInitThreadVSpace),
            SLOT_PTR(rootserver.tcb, tcbVTable));
  cteInsert(dc_ret.cap, SLOT_PTR(pptr_of_cap(root_cnode_cap), seL4_CapInitThreadIPCBuffer),
...
#ifdef CONFIG_KERNEL_MCS
#error "Not supported yet!"
  // if (!configure_sched_context(tcb, SC_PTR(rootserver.sc),
  //                              usToTicks(CONFIG_BOOT_THREAD_TIME_SLICE * US_IN_MS), 0)) {
  //   return NULL;
  // }
#endif
