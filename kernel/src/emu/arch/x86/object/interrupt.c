...
exception_t Arch_checkIRQ(word_t irq_w) {
  ...
  if (config_set(CONFIG_IRQ_IOAPIC)) {
    ...
    userError("IRQControl: IRQ %ld should be in range %ld - %ld", irq_w, (long)irq_isa_min,
              (long)irq_isa_max);
...
exception_t Arch_decodeIRQControlInvocation(word_t invLabel, word_t length, cte_t *srcSlot,
  ...
  case X86IRQIssueIRQHandlerIOAPIC: {
    ...
    return invokeIssueIRQHandlerIOAPIC(irq, ioapic, pin, level, polarity, vector, destSlot,
                                       srcSlot);
  } break;
