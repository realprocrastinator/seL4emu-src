...
exception_t performASIDControlInvocation(void *frame, cte_t *slot, cte_t *parent,
                                         asid_t asid_base) {
  ...
  cap_untyped_cap_ptr_set_capFreeIndex(
      &(parent->cap), MAX_FREE_INDEX(cap_untyped_cap_get_capBlockSize(parent->cap)));
...
BOOT_CODE bool_t map_kernel_window_devices(pte_t *pt, uint32_t num_ioapic, paddr_t *ioapic_paddrs,
  ...
  //         }
  //         pte = x86_make_device_pte(phys);
  //         assert(idx == ((PPTR_IOAPIC_START + i * BIT(PAGE_BITS)) & MASK(LARGE_PAGE_BITS)) >>
  //         PAGE_BITS); pt[idx] = pte; idx++; if (idx == BIT(PT_INDEX_BITS)) {
  //             return false;
  //         }
  //     }
  ...
  //         assert(idx == ((PPTR_DRHU_START + i * BIT(PAGE_BITS)) & MASK(LARGE_PAGE_BITS)) >>
  //         PAGE_BITS); pt[idx] = pte; idx++; if (idx == BIT(PT_INDEX_BITS)) {
  //             return false;
  //         }
  //     }
...
void unmapPage(vm_page_size_t page_size, asid_t asid, vptr_t vptr, void *pptr) {
  ...
  case X86_SmallPage:
    ...
    if (!(pte_ptr_get_present(lu_ret.ptSlot) &&
          (pte_ptr_get_page_base_address(lu_ret.ptSlot) == pptr_to_paddr(pptr)))) {
      return;
    }
  ...
  case X86_LargePage:
    ...
    pde = pd_ret.pdSlot;
    if (!(pde_ptr_get_page_size(pde) == pde_pde_large && pde_pde_large_ptr_get_present(pde) &&
          (pde_pde_large_ptr_get_page_base_address(pde) == pptr_to_paddr(pptr)))) {
      return;
    }
  ...
  invalidateTranslationSingleASID(vptr, asid, SMP_TERNARY(tlb_bitmap_get(find_ret.vspace_root), 0));
}
...
static exception_t performX86PageInvocationUnmap(cap_t cap, cte_t *ctSlot) {
  ...
  if (cap_frame_cap_get_capFMappedASID(cap)) {
    unmapPage(cap_frame_cap_get_capFSize(cap), cap_frame_cap_get_capFMappedASID(cap),
              cap_frame_cap_get_capFMappedAddress(cap), (void *)cap_frame_cap_get_capFBasePtr(cap));
  }
...
exception_t decodeX86FrameInvocation(word_t invLabel, word_t length, cte_t *cte, cap_t cap,
  ...
  case X86PageMap: { /* Map */
    ...
    default: {
      return decodeX86ModeMapPage(invLabel, frameSize, cte, cap, vspace, vaddr, paddr, vmRights,
                                  vmAttr);
    }
    }
