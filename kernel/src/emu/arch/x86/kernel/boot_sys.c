...
static BOOT_CODE bool_t try_boot_sys(void) {
  ...
  //         printf("CPU reports not vulnerable to Rogue Data Cache Load (aka Meltdown
  //         https://meltdownattack.com) "
  //                "yet SKIM window is enabled. Performance is needlessly being impacted, consider
  //                disabling.\n");
  //     } else if (!ia32_arch_capabilities_msr_get_rdcl_no(cap_msr) &&
  //     !config_set(CONFIG_KERNEL_SKIM_WINDOW)) {
  //         printf("CPU reports vulnerable to Rogue Data Cache Load (aka Meltdown
  //         https://meltdownattack.com) "
  ...
  //     if (config_set(CONFIG_KERNEL_SKIM_WINDOW) && x86_cpuid_get_identity()->vendor ==
  //     X86_VENDOR_AMD) {
  ...
  /* For the emulation always disable */
  boot_state.num_drhu = 0;

  /* Hard coded here, TODO(Jiawei): Do we need to care about this? */
  boot_state.num_cpus = 1;
...
static BOOT_CODE bool_t try_boot_sys_mbi2(multiboot2_header_t *mbi2) {
  ...
  //             printf("\tPhysical Memory Region from %lx size %lx type %u\n", m->addr, m->size,
  //             m->type); if (m->addr != (uint64_t)(word_t)m->addr) {
  //                 printf("\t\tPhysical memory region not addressable\n");
  //             }
  ...
  //         printf("Got framebuffer info in multiboot2. Current video mode is at physical
  //         address=%lx pitch=%u resolution=%ux%u@%u type=%u\n",
