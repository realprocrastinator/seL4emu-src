...
BOOT_CODE static bool_t create_untypeds(cap_t root_cnode_cap, region_t boot_mem_reuse_reg) {
  ...
  ndks_boot.bi_frame->untyped = (seL4_SlotRegion){slot_pos_before, slot_pos_after};
  return true;
}
...
BOOT_CODE static void arch_init_freemem(p_region_t ui_p_reg, v_region_t v_reg,
  ...
  init_freemem(mem_p_regs->count, mem_p_regs->list, MAX_RESERVED, reserved, v_reg,
               extra_bi_size_bits);
}

/* This function initialises a node's kernel state. It does NOT initialise the CPU. */

BOOT_CODE bool_t init_sys_state(cpu_id_t cpu_id, mem_p_regs_t *mem_p_regs, ui_info_t ui_info,
...
#error "Not implemented yet"
  ...
  create_frames_ret =
      create_frames_of_region(root_cnode_cap, it_vspace_cap, ui_reg, true, ui_info.pv_offset);
