...
void tcbDebugRemove(tcb_t *tcb) {
  ...
  if (tcb == NODE_STATE_ON_CORE(ksDebugTCBs, tcb->tcbAffinity)) {
    NODE_STATE_ON_CORE(ksDebugTCBs, tcb->tcbAffinity) =
        TCB_PTR_DEBUG_PTR(NODE_STATE_ON_CORE(ksDebugTCBs, tcb->tcbAffinity))->tcbDebugNext;
  } else {
...
static exception_t invokeConfigureSingleStepping(word_t *buffer, tcb_t *t, uint16_t bp_num,
                                                 word_t n_instrs) {
...
static exception_t invokeSetBreakpoint(tcb_t *tcb, uint16_t bp_num, word_t vaddr, word_t type,
...
static exception_t decodeSetBreakpoint(cap_t cap, word_t *buffer) {
  ...
  if (type != seL4_InstructionBreakpoint && type != seL4_DataBreakpoint) {
    ...
    if ((seL4_FirstWatchpoint == -1 || bp_num >= seL4_FirstWatchpoint) &&
      ...
      userError("Debug: Can't specify a watchpoint ID with type "
...
exception_t decodeCopyRegisters(cap_t cap, word_t length, word_t *buffer) {
  ...
  return invokeTCB_CopyRegisters(
      TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), srcTCB, flags & BIT(CopyRegisters_suspendSource),
      flags & BIT(CopyRegisters_resumeTarget), flags & BIT(CopyRegisters_transferFrame),
...
exception_t decodeReadRegisters(cap_t cap, word_t length, bool_t call, word_t *buffer) {
  ...
  if (n < 1 || n > n_frameRegisters + n_gpRegisters) {
    userError("TCB ReadRegisters: Attempted to read an invalid number of "
              "registers (%d).",
              (int)n);
    current_syscall_error.type = seL4_RangeError;
  ...
  return invokeTCB_ReadRegisters(TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)),
                                 flags & BIT(ReadRegisters_suspend), n, transferArch, call);
}
...
exception_t decodeWriteRegisters(cap_t cap, word_t length, word_t *buffer) {
  ...
  if (length - 2 < w) {
    userError("TCB WriteRegisters: Message too short for requested write size "
              "(%d/%d).",
              (int)(length - 2), (int)w);
    current_syscall_error.type = seL4_TruncatedMessage;
    return EXCEPTION_SYSCALL_ERROR;
  ...
  setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
  return invokeTCB_WriteRegisters(thread, flags & BIT(WriteRegisters_resume), w, transferArch,
                                  buffer);
}
...
#error "Not supported yet!"
  ...
  if (slotCapLongRunningDelete(TCB_PTR_CTE_PTR(cap_thread_cap_get_capTCBPtr(cap), tcbCTable)) ||
      slotCapLongRunningDelete(TCB_PTR_CTE_PTR(cap_thread_cap_get_capTCBPtr(cap), tcbVTable))) {
...
exception_t decodeSetPriority(cap_t cap, word_t length, word_t *buffer) {
  ...
  if (status != EXCEPTION_NONE) {
    userError("TCB SetPriority: Requested priority %lu too high (max %lu).", (unsigned long)newPrio,
...
#error "Not supported yet!"
  ...
  return invokeTCB_ThreadControl(TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), NULL, 0, NULL_PRIO,
                                 ...
                                 cap_null_cap_new(), NULL, thread_control_update_priority);
#endif
}
...
exception_t decodeSetMCPriority(cap_t cap, word_t length, word_t *buffer) {
  ...
  if (status != EXCEPTION_NONE) {
    userError("TCB SetMCPriority: Requested maximum controlled priority %lu "
              "too high (max %lu).",
...
#error "Not supported yet!"
  ...
  return invokeTCB_ThreadControl(TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), NULL, 0, newMcp,
                                 ...
                                 cap_null_cap_new(), NULL, thread_control_update_mcp);
#endif
}
...
#error "Not supported yet!"
  ...
  exception_t status = checkPrio(newMcp, authTCB);
  if (status != EXCEPTION_NONE) {
    userError("TCB SetSchedParams: Requested maximum controlled priority %lu "
              "too high (max %lu).",
              (unsigned long)newMcp, (unsigned long)authTCB->tcbMCP);
    return status;
  }
...
exception_t decodeDomainInvocation(word_t invLabel, word_t length, word_t *buffer) {
  ...
  if (unlikely(length == 0)) {
    ...
    if (domain >= CONFIG_NUM_DOMAINS) {
      userError("Domain Configure: invalid domain (%lu >= %u).", domain, CONFIG_NUM_DOMAINS);
      current_syscall_error.type = seL4_InvalidArgument;
...
// #ifdef CONFIG_KERNEL_MCS
// static inline exception_t installTCBCap(tcb_t *target, cap_t tCap, cte_t
// *slot,
//                                         tcb_cnode_index_t index, cap_t
//                                         newCap, cte_t *srcSlot)
// {
...
exception_t invokeTCB_ReadRegisters(tcb_t *tcb_src, bool_t suspendSource, word_t n, word_t arch,
  ...
  if (call) {
    ...
    for (i = 0;
         i < n_gpRegisters && i + n_frameRegisters < n && i + n_frameRegisters < n_msgRegisters;
         i++) {
    ...
    if (ipcBuffer != NULL && i < n_gpRegisters && i + n_frameRegisters < n) {
      for (; i < n_gpRegisters && i + n_frameRegisters < n; i++) {
        ipcBuffer[i + n_frameRegisters + 1] = getRegister(tcb_src, gpRegisters[i]);
      }
    }
...
exception_t invokeTCB_WriteRegisters(tcb_t *dest, bool_t resumeTarget, word_t n, word_t arch,
  ...
  for (i = 0; i < n_frameRegisters && i < n; i++) {
    ...
    setRegister(dest, frameRegisters[i],
                sanitiseRegister(frameRegisters[i], getSyscallArg(i + 2, buffer), archInfo));
  }

  for (i = 0; i < n_gpRegisters && i + n_frameRegisters < n; i++) {
    setRegister(dest, gpRegisters[i],
                sanitiseRegister(gpRegisters[i], getSyscallArg(i + n_frameRegisters + 2, buffer),
                                 archInfo));
  }
...
void setThreadName(tcb_t *tcb, const char *name) {
  strncpy(TCB_PTR_DEBUG_PTR(tcb)->tcbName, name, TCB_NAME_LENGTH);
}
#endif
...
word_t setMRs_syscall_error(tcb_t *thread, word_t *receiveIPCBuffer) {
  ...
  case seL4_InvalidArgument:
    return setMR(thread, receiveIPCBuffer, 0, current_syscall_error.invalidArgumentNumber);
  ...
  case seL4_FailedLookup:
    setMR(thread, receiveIPCBuffer, 0, current_syscall_error.failedLookupWasSource ? 1 : 0);
