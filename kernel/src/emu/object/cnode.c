...
exception_t invokeCNodeRevoke(cte_t *destSlot) { return cteRevoke(destSlot); }

exception_t invokeCNodeDelete(cte_t *destSlot) { return cteDelete(destSlot, true); }

exception_t invokeCNodeCancelBadgedSends(cap_t cap) {
  word_t badge = cap_endpoint_cap_get_capEPBadge(cap);
  if (badge) {
    endpoint_t UNUSED *ep = (endpoint_t *)cap_endpoint_cap_get_capEPPtr(cap);
  ...
  if ((cap_get_capType(srcCap) == cap_untyped_cap) &&
    ...
    if ((cap_untyped_cap_get_capPtr(srcCap) == cap_untyped_cap_get_capPtr(newCap)) &&
      ...
      cap_untyped_cap_ptr_set_capFreeIndex(
          &(srcSlot->cap), MAX_FREE_INDEX(cap_untyped_cap_get_capBlockSize(srcCap)));
    }
  }
}
...
void cteInsert(cap_t newCap, cte_t *srcSlot, cte_t *destSlot) {
  ...
  if (mdb_node_get_mdbNext(newMDB)) {
    mdb_node_ptr_set_mdbPrev(&CTE_PTR(mdb_node_get_mdbNext(newMDB))->cteMDBNode, CTE_REF(destSlot));
  }
}
