...
exception_t handleUnknownSyscall(word_t w) {
  ...
  if (w == SysDebugSnapshot) {
    ...
    printf("Debug snapshot syscall from user thread %p \"%s\"\n", tptr,
           TCB_PTR_DEBUG_PTR(tptr)->tcbName);
    assert("capDL call not implemented yet!");
    // capDL();
#else
...
static void handleYield(void) {
  ...
  //     /* Manually updated the scConsumed so that the full timeslice isn't added, just what was
  //     consumed */ NODE_STATE(ksCurSC)->scConsumed = consumed;
  // #else
...
//         case SysNBSendWait:
//             ret = handleInvocation(false, false, true, true, getRegister(NODE_STATE(ksCurThread),
//             replyRegister)); if (unlikely(ret != EXCEPTION_NONE)) {
//                 irq = getActiveIRQ();
//                 if (IRQT_TO_IRQ(irq) != IRQT_TO_IRQ(irqInvalid)) {
//                     mcsIRQ(irq);
//                     handleInterrupt(irq);
